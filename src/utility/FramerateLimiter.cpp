#include "../../include/utility/FramerateLimiter.hpp"
...
void FramerateLimiter::wait()
{
    if(not ResourceManager::window->isVerticalSyncEnabled())
    {
        ...
        // spdlog::trace("delta = {:.4f}, limit = {:.4f}, delay = {:.4f}", m_deltaTime, m_frameTimeLimit, delay);

        //! we cannot trust the native system scheduler, because its precision is about 10-15ms, so sleeping is not an option
