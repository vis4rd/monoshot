...
inline void drawOverlay(const GameplayLayout& layout, const std::int32_t& hero_health, const std::int32_t& max_hero_health, const std::uint32_t& current_ammo, const std::uint32_t& total_ammo, const std::uint32_t& weapon_index = 0u)
{
    ...
    const std::uint32_t weapon_texture_indices[2] = {res::rifleInventoryTexture->getID(), res::pistolInventoryTexture->getID()};
    ...
    {
        ImGui::Image((void*)(std::intptr_t)weapon_texture_indices[weapon_index], {256 * layout.viewport_w / 1920.f, 128 * layout.viewport_h / 1080.f});
